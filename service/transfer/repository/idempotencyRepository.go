...
// TransferIdempotencyRepository adapts the shared idempotency.Store contract to
// the transfer service DB's idempotency_records table.
type TransferIdempotencyRepository struct {
	db *db.Queries
}

func NewTransferIdempotencyRepository(db *db.Queries) *TransferIdempotencyRepository {
...
func mapTransferIdempotencyRecord(rec *db.IdempotencyRecord) *idempotency.Record {
	if rec == nil {
		return nil
	}
